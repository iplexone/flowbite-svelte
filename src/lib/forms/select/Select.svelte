<script lang="ts" generics="T">
  ...
  let { children, items, value = $bindable(), elementRef = $bindable(), underline, size = "md", disabled, selectClass, class: className, placeholder = "Choose option ...", clearedValue = "", clearable, clearableSvgClass, clearableColor = "none", clearableClass, clearableOnClick, ...restProps }: SelectProps<T> = $props();
...
<div class={base({ class: clsx((theme as SelectTheme)?.base, className) })}>
  <select {disabled} {...restProps} bind:value bind:this={elementRef} class={select({ class: clsx((theme as SelectTheme)?.select, selectClass) })}>
    {#if placeholder}
      <option disabled selected value={clearedValue}>{placeholder}</option>
    {/if}
  ...
  {#if value !== clearedValue && clearable}
